...
/*function get_all_post($attr, $content = null){
    $allpost = extract($_POST);
    return $allpost(); 
}
*/
//add_shortcode('output_all_post', 'get_all_post');

/* function getIp() {
    $ip = $_SERVER['REMOTE_ADDR'];
    //Checks for IP address on shared internet
    if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
        $ip = $_SERVER['HTTP_CLIENT_IP'];
    //Checks for IP on proxy server 
...
function locate_user_id(){
    $user_ip = get_feild("user_ip_address");
    return user_ip();
}

function locate_user_timezone(){
    $user_timezone = get_feild("user_timezone");
    return user_ip();
}

function save_ip_feilds(){
    
    $user_ip = get_field("user_ip_address");
    
    update_field("user_ip_address");

}

function save_timezone_field(){
    
    $user_zone = get_field("user_timezone");
    
    update_field("user_timezone");
